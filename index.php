...
if(isset($_SESSION['status']) && $_SESSION['status'] == 'verified' && !empty($_SESSION['request_vars'])){
    ...
    //Display profile image and tweet form
    $output .= '<div class="tweet_box">';
    $output .= '<img src="'.$profilePicture.'" width="50" height="40"/>';
    $output .= '<form method="post" action=""><table width="200" border="0" cellpadding="3">';
    $output .= '<p> Type Tweet :</p><tr>';
...
if(isset($myTweets->statuses) && is_array($myTweets->statuses)) {
    if(count($myTweets->statuses)) {
        foreach($myTweets->statuses as $tweet) {
            echo $tweet->text;
        }
    }
    else {
    ...
    if($twClient->http_code == '200'){
        ...
        $output = '<h1>Login to Twitter </h1></br><a href="'.filter_var($authUrl, FILTER_SANITIZE_URL).'"><img src="Images/apache_pb.png" width="300" height="100" border="0" /></a>';
    }else{
